...
export const marketingMetricsMethods = {
    calculateCAC(this: MetricsRuntime): number {
        return computeCAC(this.customers, this.marketingMetrics, this.monthStart(1).toISOString().slice(0, 7), this.monthStart(0).toISOString().slice(0, 7));
    },
    calculateChurnRate(this: MetricsRuntime): number {
        const lastMonth = this.monthStart(1);
        const thisMonth = this.monthStart(0);
        const churned = this.customers.filter((c) => c.status === "churned" && c.churn_date &&
            new Date(c.churn_date) >= lastMonth && new Date(c.churn_date) < thisMonth).length;
        const activeAtStart = this.customers.filter((c) => c.status === "active" ||
            (c.status === "churned" && c.churn_date && new Date(c.churn_date) >= lastMonth)).length;
    ...
    calculateConversionRate(this: MetricsRuntime): number {
        ...
        const monthlyMetrics = this.marketingMetrics.filter(m => m.period_start?.startsWith(lastMonthKey));
