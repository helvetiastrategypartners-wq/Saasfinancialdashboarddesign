...
export const strategyMetricsMethods = {
    simulateScenario(this: MetricsRuntime, params: Record<string, number>): Record<string, number> {
        ...
        return {
            ...
            projectedNetCashflow: Math.round(newRevenue - newBurnRate),
            projectedRunway: newBurnRate > 0
                ? Math.round((current.cash / newBurnRate) * 10) / 10
                : 999,
    ...
    calculateBreakEvenRevenue(
        ...
        return this.calculateBreakEvenThreshold(
            fixedCosts,
            this.calculateUnitMargin(price, variableCost),
        ) * price;
    },

    calculateRevenueGrowth(this: MetricsRuntime): number {
        const revM1 = sumAmounts(
    ...
    calculateGoalCompletion(this: MetricsRuntime, targetRevenue: number): number {
        return targetRevenue > 0
            ? (this.getLastMonthData().revenue / targetRevenue) * 100
            : 0;
    },
    ...
    getKPITracking(this: MetricsRuntime): Record<string, {
        ...
        for (const goal of this.goals) {
            result[goal.metric_name] = {
                ...
                completion: goal.target_value > 0
