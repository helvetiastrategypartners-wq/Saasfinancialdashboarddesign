...
const SaveButton = ({ onClick, loading, label = "Enregistrer les modifications", white = false }: { onClick: () => void; loading?: boolean; label?: string; white?: boolean }) => (
  <motion.button
    whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
    onClick={onClick}
    disabled={loading}
    className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-colors shadow-lg disabled:opacity-50 ${
      white ? "bg-white text-background hover:bg-white/90" : "bg-accent-red text-white hover:bg-accent-red/90"
    }`}
  >
...
function ProfileSettings() {
  ...
  return (
    <div className="space-y-6">
      {ToastEl}
      <SectionHeader title="Informations du Profil" subtitle="Mettez à jour vos informations personnelles" />

      <div className="flex items-center gap-6">
        <div className="w-24 h-24 rounded-full overflow-hidden flex items-center justify-center text-white text-3xl font-semibold shadow-lg"
      ...
      <div className="grid grid-cols-2 gap-6">
        <Field label="Prénom"><input type="text"  value={firstName} onChange={e => setFirstName(e.target.value)} className={inputCls} style={{ background: "var(--glass-bg)" }} /></Field>
        <Field label="Nom">   <input type="text"  value={lastName}  onChange={e => setLastName(e.target.value)}  className={inputCls} style={{ background: "var(--glass-bg)" }} /></Field>
      </div>
      <Field label="Email">     <input type="email" value={email}   onChange={e => setEmail(e.target.value)}   className={inputCls} style={{ background: "var(--glass-bg)" }} /></Field>
      <Field label="Téléphone"> <input type="tel"   value={phone}   onChange={e => setPhone(e.target.value)}   className={inputCls} style={{ background: "var(--glass-bg)" }} /></Field>
      <Field label="Entreprise"><input type="text"  value={company} onChange={e => setCompany(e.target.value)} className={inputCls} style={{ background: "var(--glass-bg)" }} /></Field>

      <SaveButton onClick={() => show("Profil enregistré avec succès")} />
    </div>
  );
}
...
function NotificationSettings() {
  ...
  return (
    <div className="space-y-6">
      {ToastEl}
      <SectionHeader title="Préférences de Notification" subtitle="Configurez comment vous souhaitez être notifié" />
      ...
      <SaveButton white onClick={() => show("Préférences de notification enregistrées")} label="Enregistrer les préférences" />
    </div>
  );
}
...
function SecuritySettings() {
  ...
  return (
    <div className="space-y-6">
      {ToastEl}
      <SectionHeader title="Sécurité" subtitle="Gérez votre mot de passe et options de sécurité" />
      <Field label="Mot de passe actuel">   <input type="password" value={current}  onChange={e => { setCurrent(e.target.value);  setError(""); }} className={inputCls} /></Field>
      <Field label="Nouveau mot de passe">  <input type="password" value={next}     onChange={e => { setNext(e.target.value);     setError(""); }} className={inputCls} /></Field>
      <Field label="Confirmer le mot de passe"><input type="password" value={confirm} onChange={e => { setConfirm(e.target.value); setError(""); }} className={inputCls} /></Field>
      ...
      <SaveButton white onClick={handleUpdate} loading={loading} label="Mettre à jour le mot de passe" />
    </div>
  );
}
...
function BillingSettings() {
  ...
  function downloadInvoice(date: string, amount: string) {
    ...
    const a    = Object.assign(document.createElement("a"), { href: url, download: `facture-${date.replace(/\s/g, "-")}.txt` });
    a.click(); URL.revokeObjectURL(url);
  }

  function savePlan() { setShowPlanModal(false); show(`Plan mis à jour : ${PLANS.find(p => p.id === selectedPlan)?.name}`); }
  function saveCard()  { setShowCardModal(false); show("Moyen de paiement mis à jour"); setCardNumber(""); setCardExpiry(""); setCardCvc(""); }

  return (
    <div className="space-y-6">
      {ToastEl}
      <SectionHeader title="Facturation" subtitle="Gérez votre abonnement et moyens de paiement" />
      ...
      <div className="p-6 rounded-xl bg-white/5 border border-glass-border">
        ...
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowPlanModal(true)}
      ...
      <div>
        ...
        <div className="p-6 rounded-xl bg-white/5 border border-glass-border flex items-center justify-between">
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowCardModal(true)}
            className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-foreground transition-colors">
            Modifier
          </motion.button>
        </div>
          ...
          {[
            ...
            <div key={i} className="p-4 rounded-lg bg-white/5 border border-glass-border flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => downloadInvoice(inv.date, inv.amount)}
        ...
        {showPlanModal && (
          <Overlay onClose={() => setShowPlanModal(false)} title="Changer de plan">
            <div className="space-y-3 mb-6">
              {PLANS.map(plan => (
                <motion.div key={plan.id} whileHover={{ scale: 1.01 }} onClick={() => setSelectedPlan(plan.id)}
            ...
            <div className="flex justify-end gap-3">
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={savePlan}
                className="px-6 py-2.5 rounded-xl bg-accent-red text-white hover:bg-accent-red/90 transition-colors">Confirmer</motion.button>
            </div>
          </Overlay>
        )}
        {showCardModal && (
          <Overlay onClose={() => setShowCardModal(false)} title="Modifier le moyen de paiement" small>
            <div className="space-y-4 mb-6">
              <Field label="Numéro de carte"><input type="text" value={cardNumber} onChange={e => setCardNumber(e.target.value)} placeholder="1234 5678 9012 3456" maxLength={19} className={inputCls} /></Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Expiration"><input type="text" value={cardExpiry} onChange={e => setCardExpiry(e.target.value)} placeholder="MM/YY" maxLength={5} className={inputCls} /></Field>
                <Field label="CVC">       <input type="text" value={cardCvc}    onChange={e => setCardCvc(e.target.value)}    placeholder="123"   maxLength={3} className={inputCls} /></Field>
              </div>
            </div>
            <div className="flex justify-end gap-3">
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={saveCard} disabled={!cardNumber || !cardExpiry || !cardCvc}
                className="px-6 py-2.5 rounded-xl bg-accent-red text-white hover:bg-accent-red/90 transition-colors disabled:opacity-50">Enregistrer</motion.button>
            </div>
          </Overlay>
        )}
      </AnimatePresence>
...
function PreferenceSettings() {
  ...
  return (
    <div className="space-y-6">
      {ToastEl}
      <SectionHeader title="Préférences" subtitle="Personnalisez votre expérience" />
      <Field label="Langue">
        <select value={lang} onChange={e => setLang(e.target.value)} className={inputCls}>
          <option>Français</option><option>English</option><option>Deutsch</option><option>Italiano</option>
        </select>
      </Field>
      <Field label="Devise">
        <select value={currency} onChange={e => setCurrency(e.target.value)} className={inputCls}>
          <option>CHF (Franc Suisse)</option><option>EUR (Euro)</option><option>USD (Dollar Américain)</option>
        </select>
      </Field>
      <Field label="Fuseau horaire">
        <select value={tz} onChange={e => setTz(e.target.value)} className={inputCls}>
          <option>Europe/Zurich (GMT+1)</option><option>Europe/Paris (GMT+1)</option><option>Europe/London (GMT+0)</option>
        </select>
      </Field>
      <div className="space-y-4 pt-4 border-t border-glass-border">
      ...
      <SaveButton white onClick={() => show("Préférences enregistrées avec succès")} label="Enregistrer les préférences" />
    </div>
  );
}
...
export function Settings() {
  ...
  return (
    <div className="p-8 space-y-6">
      <PageHeader title="Paramètres" subtitle="Gérez vos préférences et configuration" />
      ...
      <div className="grid grid-cols-4 gap-6">
        ...
        <motion.div
          ...
          className="col-span-3 rounded-2xl p-8 backdrop-blur-xl border border-glass-border"
